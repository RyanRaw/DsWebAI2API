...
export class DeepSeekWebClient {
    ...
    constructor(cred: Partial<DeepSeekCredentials> | string) {
        if (typeof cred === "string") {
            cred = JSON.parse(cred);
            if (typeof cred === "string") cred = { cookie: cred };
        }
        this.headers = {
            Cookie: cred.cookie ?? "",
            "User-Agent": cred.userAgent ?? "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
            "Content-Type": "application/json",
            Accept: "*/*",
            ...(cred.bearer ? { Authorization: `Bearer ${cred.bearer}` } : {}),
    ...
    async getBanner() {
        if (this.verbose) console.log(`[DeepSeekWebClient] Fetching banner...`);
        ...
        if (!res.ok) {
            console.error(`[DeepSeekWebClient] Failed to get banner: ${res.status}`, await res.text());
            throw new Error(`Failed to get banner: ${res.status}`);
        }
        return await res.json();
    }

    // 生成 PoW 挑战 这个挑战是有期限的，过期后需要重新生成，只能用一次
    async createPowChallenge(targetPath: string): Promise<DeepSeekPowChallenge> {
        if (this.verbose) console.log(`[DeepSeekWebClient] Creating PoW challenge for ${targetPath}...`);
        ...
        if (this.verbose) console.log(`[DeepSeekWebClient] Challenge extracted successfully:`, challenge);
        return challenge;
    }
    ...
    async solvePow(challenge: DeepSeekPowChallenge): Promise<number> {
        ...
        if (this.verbose) console.log(`[DeepSeekWebClient] Solving PoW (${algorithm}, difficulty: ${difficulty})...`);
    ...
    // 网页上是先创建session，缓存了id，供下一次对话使用
    // 每次对话时的create其实是给下一次用的，所以刷洗也会请求一次
    // 这个请求不需要pow
    async createChatSession(): Promise<string> {
        ...
        if (!sessionId) {
            console.error(`[DeepSeekWebClient] Critical Error: Chat session ID missing in response! ${data}`);
            throw new Error(`Chat session ID missing in response: ${JSON.stringify(data)}`);
        }
        if (this.verbose) console.log(`[DeepSeekWebClient] Chat session created: ${sessionId}`);
        return sessionId;
    }
    ...
    async chatCompletions(params: ServerChatRequest & {sessionId: string}): Promise<ReadableStream<Uint8Array>> {
        ...
        if (this.verbose) console.log(
            `[DeepSeekWebClient] Sending chat completion request (session: ${params.sessionId}, parent_message_id: ${params.parentMessageId})...`
        );
    ...
    async uploadFile(
        ...
        while (attempts < 30) {
            ...
            if (pollRes.ok) {
                ...
                if (file?.status === "SUCCESS") {
                    if (this.verbose) console.log(`[DeepSeekWebClient] File ${fileName} uploaded successfully with fileId: ${fileId}`);
                    return fileId;
                }
        ...
        if (this.verbose) console.warn(`[DeepSeekWebClient] File upload polling timed out for ${fileId}`);
        return fileId;
    }

    async currentUser() {
        if (this.verbose) console.log(`[DeepSeekWebClient] Fetching current user info...`);
        ...
        if (!res.ok) {
            const errorText = await res.text();
            console.error(`[DeepSeekWebClient] Failed to get current user: ${res.status}`, errorText);
            throw new Error(`Failed to get current user: ${res.status} ${errorText}`);
        }
        const data = await res.json();
        if (this.verbose) console.log(`[DeepSeekWebClient] Current user info fetched successfully.`);
        return data;
    }

    async getHistory(sessionId: string) {
        if (this.verbose) console.log(`[DeepSeekWebClient] Fetching chat history for session ${sessionId}...`);
        ...
        if (!res.ok) {
            const errorText = await res.text();
            console.error(`[DeepSeekWebClient] Failed to get chat history: ${res.status}`, errorText);
            throw new Error(`Failed to get chat history: ${res.status} ${errorText}`);
        }

        const data = await res.json();
        if (this.verbose) console.log(`[DeepSeekWebClient] Chat history fetched successfully.`);
        return data;
    }

    async getAllSections() {
        if (this.verbose) console.log(`[DeepSeekWebClient] Fetching all sections...`);
        const res = await fetch('https://chat.deepseek.com/api/v0/chat_session/fetch_page?lte_cursor.pinned=false', {
            method: 'GET',
            headers: this.headers
        });
        if (!res.ok) {
            const errorText = await res.text();
            console.error(`[DeepSeekWebClient] Failed to get sections: ${res.status}`, errorText);
            throw new Error(`Failed to get sections: ${res.status} ${errorText}`);
        }
        const data = await res.json();
        if (this.verbose) console.log(`[DeepSeekWebClient] Sections fetched successfully.`);
        return data;
    }

    async deleteSession(sessionId: string) {
        if (this.verbose) console.log(`[DeepSeekWebClient] Deleting chat session ${sessionId}...`);
        ...
        if (!res.ok) {
            const errorText = await res.text();
            console.error(`[DeepSeekWebClient] Failed to delete session: ${res.status}`, errorText);
            throw new Error(`Failed to delete session: ${res.status} ${errorText}`);
        }
        const data = await res.json();
        if (this.verbose) console.log(`[DeepSeekWebClient] Session ${sessionId} deleted successfully.`);
        return data;
    }

    async logout() {
        if (this.verbose) console.log(`[DeepSeekWebClient] Logging out...`);
        const res = await fetch('https://chat.deepseek.com/api/v0/users/logout', {
            method: 'POST',
            headers: this.headers
        });
        if (!res.ok) {
            const errorText = await res.text();
            console.error(`[DeepSeekWebClient] Failed to logout: ${res.status}`, errorText);
            throw new Error(`Failed to logout: ${res.status} ${errorText}`);
        }
        const data = await res.json();
        if (this.verbose) console.log(`[DeepSeekWebClient] Logged out successfully.`);
        return data;
    }
}